...
import { eventResultsSourceRepo, eventZpSourceRepo } from '../database/complete-source-repositories.js';
...
export class MVPEventEnricherService {
  ...
  constructor() {
    ...
    // Use the singleton instances from complete-source-repositories
  }
  ...
  async getUnenrichedEvents(limit: number = 50): Promise<number[]> {
    ...
    for (const event of events) {
      const hasResults = await eventResultsSourceRepo.hasEventData(event.id);
      const hasZp = await eventZpSourceRepo.hasEventZpData(event.id);
