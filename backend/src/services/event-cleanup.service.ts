...
export class EventCleanupService {
  ...
  async cleanupOldEvents(): Promise<{
    ...
    const eventsToKeep: Array<{ id: number; event_id: string; reason: string }> = [];
    ...
    for (const event of pastEvents) {
      ...
      const hasTeamParticipants = signups?.some(s => 
        teamZwiftIds.has(s.rider_id.toString())
      ) || false;
   ...
   */
  async cleanupStaleUpcomingEvents(): Promise<number> {
    const now = Math.floor(Date.now() / 1000);
    ...
    for (const event of futureEvents) {
      ...
      const hasTeamSignups = signups?.some(s => 
        teamZwiftIds.has(s.rider_id.toString())
      ) || false;
