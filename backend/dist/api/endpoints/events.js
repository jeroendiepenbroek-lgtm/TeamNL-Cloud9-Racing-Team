...
router.get('/', async (req, res) => {
    try {
        const clubId = req.query.clubId ? parseInt(req.query.clubId) : undefined;
...
router.get('/upcoming', async (req, res) => {
    try {
        ...
        const hours = req.query.hours ? parseInt(req.query.hours) : config.lookforwardHours;
        ...
        const eventIds = baseEvents.map((e) => String(e.event_id));
        ...
        const enrichedEvents = baseEvents.map((event) => {
            ...
            const teamSignupsByCategory = {};
            teamSignups.forEach(signup => {
                let pen = signup.pen_name || 'Unknown';
                // Merge A+ into A
                if (pen === 'A+')
                    pen = 'A';
                if (!teamSignupsByCategory[pen])
                    teamSignupsByCategory[pen] = [];
...
router.get('/debug', async (req, res) => {
    try {
        ...
        if (error)
            throw error;
        const now = Math.floor(Date.now() / 1000);
        const eventsWithTime = (data || []).map((e) => ({
            ...e,
...
router.post('/sync/rider-events', async (req, res) => {
    try {
