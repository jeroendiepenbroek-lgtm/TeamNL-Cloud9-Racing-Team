...
router.get('/team', async (req, res) => {
    try {
        ...
        const { data: clubMembersCount } = await supabase.client
            .from('club_members')
            .select('*', { count: 'exact', head: true });
        const { data: ridersCount } = await supabase.client
            .from('riders')
            .select('*', { count: 'exact', head: true });
...
router.get('/:zwiftId', async (req, res) => {
    try {
...
router.post('/team/bulk', async (req, res) => {
    try {
...
router.delete('/team/:zwiftId', async (req, res) => {
    try {
        const zwiftId = parseInt(req.params.zwiftId);
...
router.put('/team/:zwiftId/favorite', async (req, res) => {
    try {
        const zwiftId = parseInt(req.params.zwiftId);
