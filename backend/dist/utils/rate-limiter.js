...
export class RateLimiter {
    ...
    canMakeCall(endpoint) {
        ...
        const now = Date.now();
        const recentCalls = history.filter(call => now - call.timestamp < config.windowMs);
    ...
    async _waitForSlotInternal(endpoint) {
        const config = this.configs[endpoint];
        while (!this.canMakeCall(endpoint)) {
            ...
            if (waitTime > 0) {
                ...
                console.log(`[RateLimiter] 🚦 ${friendlyName}: ` +
     ...
     */
    async executeWithLimit(endpoint, callback) {
