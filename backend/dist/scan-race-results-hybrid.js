 ...
 */
async function scrapeRecentEvents(lookbackDays) {
 ...
 */
async function pollRidersForEvents(supabase, lookbackDays) {
    ...
    try {
        ...
        for (let i = 0; i < myRiders.length; i++) {
            ...
            try {
                const riderResp = await axios.get(`https://api.zwiftracing.app/api/public/riders/${rider.rider_id}`, { headers: API_HEADERS, timeout: 10000 });
 ...
 */
async function fetchAndSaveEvents(eventIds, supabase, teamRiderIds) {
    console.log(`\\n📊 Fetching ${eventIds.length} events...`);
    let scanned = 0;
    let saved = 0;
    for (const eventId of eventIds) {
        try {
            const response = await axios.get(`https://api.zwiftracing.app/api/public/results/${eventId}`, { headers: API_HEADERS, timeout: 10000 });
            scanned++;
            if (response.data?.results?.length > 0) {
                const teamResults = response.data.results.filter((r) => teamRiderIds.has(r.riderId));
 ...
 */
export async function scanRaceResultsHybrid(supabase, lookbackDays = 7) {
