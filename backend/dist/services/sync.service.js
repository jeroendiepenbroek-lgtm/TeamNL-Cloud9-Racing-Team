...
export class SyncService {
     ...
     */
    async syncClub(clubId = TEAM_CLUB_ID) {
     ...
     */
    async syncRiders(clubId = TEAM_CLUB_ID) {
     ...
     */
    async syncEvents(clubId = TEAM_CLUB_ID) {
     ...
     */
    async syncEventResults(eventId) {
     ...
     */
    async syncRiderHistory(riderId) {
     ...
     */
    async syncAll(clubId = TEAM_CLUB_ID) {
     ...
     */
    async syncRiderUpcomingEvents(hours = 48) {
    ...
    async bulkImportUpcomingEvents() {
        ...
        try {
            ...
            const riderMap = new Map(ourRiders.map((r) => [r.rider_id, r]));
            ...
            for (const event of events) {
                try {
                    ...
                    const apiEventData = {
                        ...
                        elevation_meters: (() => {
                            if (event.elevation)
                                return Math.floor(parseFloat(String(event.elevation))); // FORCE floor to integer
                            const routeId = event.routeId || event.route_id;
                            if (routeId) {
                        ...
                        route_id: event.routeId || event.route_id || null, // Store routeId from API
                        // Route info: gebruik cached route data als event.route NULL is
                        route_name: (() => {
                            if (event.route?.name)
                                return event.route.name;
                            const routeId = event.routeId || event.route_id;
                            return routeId ? (zwiftClient.getCachedRouteById(routeId)?.name || null) : null;
                        })(),
                        route_world: (() => {
                            if (event.route?.world)
                                return event.route.world;
                            const routeId = event.routeId || event.route_id;
                        ...
                        route_full: (() => {
                            if (event.route)
                                return JSON.stringify(event.route);
                            const routeId = event.routeId || event.route_id;
                            if (routeId) {
                                const cachedRoute = zwiftClient.getCachedRouteById(routeId);
