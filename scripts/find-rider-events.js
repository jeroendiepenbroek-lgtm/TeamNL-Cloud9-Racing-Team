...
async function findRiderEvents(zwiftId, startEventId = 5200000, // Recent events (adjust based on current date)
eventCount = 100) {
    ...
    for (let eventId = startEventId; eventId < startEventId + eventCount; eventId++) {
        try {
            ...
            const riderResult = results.find((r) => r.riderId === zwiftId);
            if (riderResult) {
                const event = {
                    eventId,
...
readline.question('Doorgaan? (yes/no): ', (answer) => {
    readline.close();
...
//# sourceMappingURL=find-rider-events.js.map