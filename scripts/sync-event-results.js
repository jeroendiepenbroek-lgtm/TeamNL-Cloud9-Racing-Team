...
async function syncEventResults(eventId) {
    ...
    try {
        ...
        const event = await prisma.event.upsert({
            ...
            create: {
                ...
                totalFinishers: results.filter((r) => r.time !== 21600).length, // 21600 = DNF
                dataSource: 'manual',
            },
            update: {
                totalParticipants: results.length,
                totalFinishers: results.filter((r) => r.time !== 21600).length,
            },
        });
        ...
        // Check matches
        const matchedResults = results.filter((r) => riderMap.has(r.riderId));
        ...
        logger.info(`Finishers: ${results.filter((r) => r.time !== 21600).length}`);
        logger.info(`DNFs: ${results.filter((r) => r.time === 21600).length}`);
        // Category breakdown
...
//# sourceMappingURL=sync-event-results.js.map