 ...
 */
async function findRiderEvents(): Promise<EventInfo[]> {
  try {
    ...
    if (response.data.recentEvents && Array.isArray(response.data.recentEvents)) {
      ...
      return response.data.recentEvents.map((evt: any) => ({
 ...
 */
async function syncEvent(eventId: number, eventName: string) {
  try {
    logger.info(`\n🏁 Sync event ${eventId}: ${eventName}`);
    
    // Gebruik de API endpoint van onze server
    const response = await axios.post(
      `http://localhost:${config.port}/api/sync/event/${eventId}`,
      {},
      { timeout: 120000 } // 2 minuten timeout voor grote events
    );
