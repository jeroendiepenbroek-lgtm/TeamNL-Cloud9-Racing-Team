...
async function listAllRiders() {
  ...
  try {
    ...
    const byClub = riders.reduce((acc, rider) => {
      ...
      return acc;
    }, {} as Record<string, typeof riders>);
    ...
    Object.entries(byClub).forEach(([clubId, clubRiders]) => {
      ...
      clubRiders.forEach((rider, idx) => {
        const stats = rider.statistics as any;
...
async function addRider(providedZwiftId?: number) {
...
async function updateRider(providedZwiftId?: number) {
  let zwiftId = providedZwiftId;
...
async function showDatabaseStats() {
  ...
  try {
    ...
    const totalRaces = riders.reduce((sum, r) => {
      const stats = r.statistics as any;
...
async function syncRider(providedZwiftId?: number) {
  let zwiftId = providedZwiftId;
