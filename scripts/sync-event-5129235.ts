...
async function syncEventDirect() {
  ...
  try {
    ...
    const validResults = response.data.map((result: any, index: number) => {
      try {
    ...
    if (targetRiderResult) {
      ...
      if (targetRiderResult.bikeFrame || targetRiderResult.bikeWheels) {
        logger.info(`\n  Bike Setup:`);
        if (targetRiderResult.bikeFrame) logger.info(`    Frame: ${targetRiderResult.bikeFrame}`);
        if (targetRiderResult.bikeWheels) logger.info(`    Wheels: ${targetRiderResult.bikeWheels}`);
      }
