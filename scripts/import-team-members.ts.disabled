 ...
 */
function parseCSV(filePath: string): RiderImport[] {
 ...
 */
function parseJSON(filePath: string): RiderImport[] {
...
async function main() {
  ...
  try {
    ...
    const results = {
      added: [] as number[],
      skipped: [] as number[],
      failed: [] as Array<{ zwiftId: number; error: string }>,
    };
      ...
      for (const rider of batch) {
        ...
        try {
          ...
          await teamService.addMember(
            options.teamId,
            rider.zwiftId,
            rider.role || options.role || 'member',
            rider.notes
          );
